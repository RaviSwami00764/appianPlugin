...
let codeSetupGridColumn=(str)=>{
    ...
    a = str.split(',');
    valueToSet = matchVal.length>1?`$match ${matchVal} fv!row.${a[0]}`:iconVal.length>1?`${iconVal} fv!row.${a[0]}`:`$ fv!row.${a[0]}`;
...
const getGridColumn = ()=>{
    ...
    for(content of data){
        ...
        arr = arr.map((x)=>{return codeSetupGridColumn(x)});
        arr = arr.join(',\n');
